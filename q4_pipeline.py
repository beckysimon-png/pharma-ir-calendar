...
if __name__ == "__main__":
    ...
    fresh_uids = {ev.uid for ev in all_kept}
    failed_names = {name for name, _ in failures}
    today = dt.datetime.now(dt.timezone.utc).date()
    carried, carry_counts, archived = _events_to_retain(
        "pharma_events.ics", fresh_uids, failed_names, today)
    for name, n in carry_counts.items():
        print(f"  carry-forward: retained {n} previous event(s) "
              f"for {name} (fetch failed this run).")
    if archived:
        print(f"  history archive: retained {archived} past event(s) that have "
              f"aged out of the live fetch (permanent historical record).")
