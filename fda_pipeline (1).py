...
def fetch_fda(cfg: dict | None = None) -> str:
    ...
    # Live path uses only the Python standard library (urllib) so this module
    # has ZERO third-party dependencies and needs no pip install in CI.
    ...
    try:
        ...
        raise RuntimeError(f"FDA: GET {CALENDAR_URL} -> HTTP {e.code}")
    except urllib.error.URLError as e:
        raise RuntimeError(f"FDA: GET {CALENDAR_URL} failed -> {e.reason}")
    if status != 200:
        raise RuntimeError(f"FDA: GET {CALENDAR_URL} -> HTTP {status}")
    print(f"FDA: fetched live calendar ({len(text)} bytes).")
    return text
