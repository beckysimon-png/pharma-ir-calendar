...
def parse_fda(raw_json, cfg: dict | None = None) -> list:
    ...
    for d in docs:
        ...
        # ---- Full-text enrichment (the ODAC fix) ------------------------------
        # The drug/sponsor for drug-committee meetings lives in the notice BODY,
        # not the API abstract. For UPCOMING kept meetings, fetch the full text
        # and mine its Agenda section. Gated by FDA_NO_ENRICH (tests/offline) and
        # fully graceful: any failure -> keep the abstract-based title.
        if _enrich_enabled() and start_d >= _today():
            body = _fetch_text(_clean_text(d.get("raw_text_url")))
            if body:
                body_topic = _extract_topic_from_body(body)
                if body_topic:
                    topic = body_topic            # body names the drug -> prefer it
