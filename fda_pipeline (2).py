...
def _extract_topic(abstract: str) -> str:
    """Short 'what is being discussed' phrase for the event TITLE."""
    a = _clean_text(abstract)
    if not a:
        return ""
    m = re.search(
        r"(?:will |to )?discuss(?:ion)?(?: and make recommendations)?"
        r"(?: on| of)?\s+(.+)", a, re.I)
    topic = (m.group(1) if m else a).strip()
    topic = re.split(r"(?<=[.;])\s", topic)[0].strip().rstrip(".")
    if len(topic) > 160:
