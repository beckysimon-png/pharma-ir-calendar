...
_NO_APPNO  = []   # CBER/CDER meeting but no specific NDA/BLA -> dropped (drugs-only filter)
...
# Detects a SPECIFIC drug application anywhere in the text: spelled-out
# ("New Drug Application 220359", "...Application (NDA) 220359") OR abbreviated
# ("NDA 220359", "BLA 125768", "sNDA 218197/S-004", "EUA 27034"). Used to keep
# ONLY meetings tied to a specific NDA/BLA and drop vaccine-formula / strain-
# selection / policy-topic meetings that name no application number.
_APPNO_ANYWHERE = re.compile(
    r"(?:(?:supplemental\s+)?(?:new\s+drug|biologics?\s+license|"
    r"abbreviated\s+new\s+drug)\s+application|\b(?:s?BLA|s?NDA|ANDA|EUA)\b)"
    r"\)?[\s#:()A-Za-z]{0,8}\d{4,6}", re.I)


def _has_application_number(*texts) -> bool:
    """True if any of the given text chunks references a specific NDA/BLA/etc."""
    joined = " ".join(t for t in texts if t)
    return bool(_APPNO_ANYWHERE.search(joined))
...
def _drugs_only_enabled() -> bool:
    """Keep ONLY meetings tied to a specific drug application (NDA/BLA/etc.).
    ON by default; set FDA_ALL_MEETINGS=1 to keep every CBER/CDER meeting."""
    return os.environ.get("FDA_ALL_MEETINGS") != "1"
...
def parse_fda(raw_json, cfg: dict | None = None) -> list:
    ...
    events = []
    _UNDATED.clear(); _CANCELLED.clear(); _NO_APPNO.clear()
    if not raw_json:
        return events
    ...
    for d in docs:
        ...
        # ---- DRUGS-ONLY FILTER -----------------------------------------------
        # Keep only meetings tied to a SPECIFIC drug application. We check every
        # signal we have: the title + abstract (free, present for all meetings)
        # plus any drug/topic pulled from the title or the fetched body. If no
        # NDA/BLA/ANDA/EUA is named anywhere, it is a vaccine-formula / strain-
        # selection / policy-topic meeting -> drop it (surfaced in the log).
        if _drugs_only_enabled() and not _has_application_number(
                title, abstract, drug, body_topic):
            _NO_APPNO.append(committee)
            continue
...
def main():
    ...
    if _NO_APPNO:
        ...
        _c = Counter(_NO_APPNO)
        _summary = ", ".join(f"{name} x{n}" if n > 1 else name
                             for name, n in _c.most_common())
        print(f"  note: dropped {len(_NO_APPNO)} CBER/CDER meeting(s) with no "
              f"specific NDA/BLA (drugs-only filter; set FDA_ALL_MEETINGS=1 to "
              f"keep): {_summary}")
