...
Q4_DEFAULT_BODY = {
    "serviceDto": {
        "ViewType": "1",           # 1 = upcoming+past; "2" = upcoming only on some sites
        "ViewDate": "",
        "RevisionNumber": 0,
        "LanguageId": 1,
        "Signature": "",
        "ItemCount": -1,           # -1 = return everything
        "StartIndex": 0,
        "TagList": [],
        "IncludeTags": False,
    },
    "year": None,                  # None/absent = all years
}
...
COMPANIES = {
    "pfizer": {
        ...
        # <-- CONFIRM this from the DevTools "Headers" tab (Request URL). This is
        #     the standard Q4 path; the real host is sometimes a q4web subdomain.
        "endpoint": "https://investors.pfizer.com/feed/Event.svc/GetEventList",
        "method": "POST",          # confirm in Headers tab (usually POST)
        "body": Q4_DEFAULT_BODY,   # confirm in Payload tab; None for a GET
    },
    ...
    #            "method": "POST", "body": Q4_DEFAULT_BODY},
...
def fetch_q4(cfg: dict) -> dict:
    ...
    headers = {
        "User-Agent": "pharma-ir-calendar/1.0 (+github actions)",
        "Accept": "application/json, text/plain, */*",
        "Content-Type": "application/json",
        # Some Q4 sites check Referer/Origin; harmless to send:
        "Referer": cfg["ir_host"],
        "Origin": cfg["ir_host"],
    }
